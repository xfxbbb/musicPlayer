...
#include "tools/get_vulkan_proc_address.h"
...
namespace swapchain {
...
#if defined(__ANDROID__)
    void write_warning(const char* message) { __android_log_print(ANDROID_LOG_WARN, "VirtualSwapchainLayer", "%s", message); }
#else
    void write_warning(const char* message) { std::cerr << "VirtualSwapchainLayer: " << message << std::endl; }
#endif

    void write_warning(const std::string& message) { write_warning(message.c_str()); }

    bool GetParameter(const char* const env_var_name, const char* const android_prop_name, std::string* param_value) {
#if defined(__ANDROID__)
    ...
    namespace {
        ...
        struct link_info_traits {
            const static bool is_instance = std::is_same<T, const VkInstanceCreateInfo>::value;
            using layer_info_type = typename std::conditional<is_instance, VkLayerInstanceCreateInfo, VkLayerDeviceCreateInfo>::type;
            const static VkStructureType sType =
                is_instance ? VK_STRUCTURE_TYPE_LOADER_INSTANCE_CREATE_INFO : VK_STRUCTURE_TYPE_LOADER_DEVICE_CREATE_INFO;
        };
        ...
        // passed in.
        template <typename T>
        typename link_info_traits<T>::layer_info_type* get_layer_link_info(T* pCreateInfo) {
            using layer_info_type = typename link_info_traits<T>::layer_info_type;

            auto layer_info = const_cast<layer_info_type*>(static_cast<const layer_info_type*>(pCreateInfo->pNext));

            while (layer_info) {
                if (layer_info->sType == link_info_traits<T>::sType && layer_info->function == VK_LAYER_LINK_INFO) {
                    return layer_info;
                }
                layer_info = const_cast<layer_info_type*>(static_cast<const layer_info_type*>(layer_info->pNext));
            }
            return layer_info;
        }
    } // namespace
    ...
    VKAPI_ATTR VkResult VKAPI_CALL vkCreateInstance(const VkInstanceCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator,
        ...
        // PFN_vkGetInstanceProcAddr get_instance_proc_addr = layer_info->u.pLayerInfo->pfnNextGetInstanceProcAddr;

        PFN_vkGetInstanceProcAddr get_instance_proc_addr = reinterpret_cast<PFN_vkGetInstanceProcAddr>(core::GetVulkanInstanceProcAddress);

        // From that get the next vkCreateInstance function.
        PFN_vkCreateInstance create_instance = reinterpret_cast<PFN_vkCreateInstance>(get_instance_proc_addr(NULL, "vkCreateInstance"));
        ...
        PFN_vkEnumeratePhysicalDevices enumerate_physical_devices =
            reinterpret_cast<PFN_vkEnumeratePhysicalDevices>(get_instance_proc_addr(*pInstance, "vkEnumeratePhysicalDevices"));
        if (!enumerate_physical_devices) {
            return VK_ERROR_INITIALIZATION_FAILED;
        }

        PFN_vkEnumerateDeviceExtensionProperties enumerate_device_extension_properties =
            reinterpret_cast<PFN_vkEnumerateDeviceExtensionProperties>(get_instance_proc_addr(*pInstance, "vkEnumerateDeviceExtensionProperties"));
...
#define GET_PROC(name) data.name = reinterpret_cast<PFN_##name>(get_instance_proc_addr(*pInstance, #name))
...
#undef GET_PROC
    ...
    VKAPI_ATTR VkResult VKAPI_CALL vkCreateDevice(VkPhysicalDevice gpu, const VkDeviceCreateInfo* pCreateInfo,
        ...
        PFN_vkGetInstanceProcAddr get_instance_proc_addr = reinterpret_cast<PFN_vkGetInstanceProcAddr>(core::GetVulkanInstanceProcAddress);
        auto instances = GetGlobalContext().GetInstanceMap();
        PFN_vkCreateDevice create_device = core::get_vulkan_fun<PFN_vkCreateDevice>("vkCreateDevice");
        ...
        PFN_vkGetDeviceProcAddr get_device_proc_addr = core::get_vulkan_fun<PFN_vkGetDeviceProcAddr>("vkGetDeviceProcAddr");
...
#define GET_PROC(name) data.name = reinterpret_cast<PFN_##name>(get_device_proc_addr(*pDevice, #name));
...
#undef GET_PROC
    ...
    VkResult get_layer_properties(uint32_t* pPropertyCount, VkLayerProperties* pProperties) {
        if (pProperties == NULL) {
            *pPropertyCount = 1;
            return VK_SUCCESS;
        }
    ...
    VKAPI_ATTR VkResult VKAPI_CALL vkEnumeratePhysicalDevices(VkInstance instance, uint32_t* pPhysicalDeviceCount,
        ...
        if (instance_data->physical_devices_.empty()) {
            uint32_t count;
            VkResult res = instance_data->vkEnumeratePhysicalDevices(instance, &count, nullptr);
            if (res != VK_SUCCESS) {
                return res;
            }
            instance_data->physical_devices_.resize(count);
            if (VK_SUCCESS != (res = instance_data->vkEnumeratePhysicalDevices(instance, &count, instance_data->physical_devices_.data()))) {
        ...
        if (pPhysicalDevices) {
            ...
            memcpy(pPhysicalDevices, instance_data->physical_devices_.data(), *pPhysicalDeviceCount * sizeof(VkPhysicalDevice));
        } else {
            *pPhysicalDeviceCount = count;
        }
        return VK_SUCCESS;
    ...
    VKAPI_ATTR VkResult VKAPI_CALL vkAllocateCommandBuffers(VkDevice device, const VkCommandBufferAllocateInfo* pAllocateInfo,
        ...
        VkResult res = device_data->vkAllocateCommandBuffers(device, pAllocateInfo, pCommandBuffers);
        if (res == VK_SUCCESS) {
            for (size_t i = 0; i < pAllocateInfo->commandBufferCount; ++i) {
                (*command_buffer_map)[pCommandBuffers[i]] = {device, device_data->vkCmdPipelineBarrier, device_data->vkCmdWaitEvents};
            }
        }
        return res;
    }
    ...
    VKAPI_ATTR VkResult VKAPI_CALL vkEnumerateDeviceExtensionProperties(VkPhysicalDevice physicalDevice, const char* pLayerName,
        ...
        auto instance_data = GetGlobalContext().GetInstanceData(GetGlobalContext().GetPhysicalDeviceData(physicalDevice)->instance_);

        return instance_data->vkEnumerateDeviceExtensionProperties(physicalDevice, pLayerName, pPropertyCount, pProperties);
    }
    ...
    VKAPI_ATTR void VKAPI_CALL vkFreeCommandBuffers(VkDevice device, VkCommandPool commandPool, uint32_t commandBufferCount,
        ...
        GetGlobalContext().GetDeviceData(device)->vkFreeCommandBuffers(device, commandPool, commandBufferCount, pCommandBuffers);
    }
...
#undef INTERCEPT_SURFACE
        ...
        // PFN_vkGetInstanceProcAddr instance_proc_addr = GetGlobalContext().GetInstanceData(instance)->vkGetInstanceProcAddr;
        // PFN_vkGetInstanceProcAddr instance_proc_addr = reinterpret_cast<PFN_vkGetInstanceProcAddr>(core::GetVulkanInstanceProcAddress);
        PFN_vkGetInstanceProcAddr instance_proc_addr = core::get_vulkan_fun<PFN_vkGetInstanceProcAddr>("vkGetInstanceProcAddr");
...
#undef INTERCEPT
        ...
        // PFN_vkGetDeviceProcAddr device_proc_addr = GetGlobalContext().GetDeviceData(dev)->vkGetDeviceProcAddr;
        PFN_vkGetDeviceProcAddr device_proc_addr = core::get_vulkan_fun<PFN_vkGetDeviceProcAddr>("vkGetDeviceProcAddr");
...
VK_LAYER_EXPORT VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL LAYER_NAME_FUNCTION(GetDeviceProcAddr)(VkDevice dev, const char* funcName) {
    return swapchain::vkGetDeviceProcAddr(dev, funcName);
}

VK_LAYER_EXPORT VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL LAYER_NAME_FUNCTION(GetInstanceProcAddr)(VkInstance instance, const char* funcName) {
...
#if defined(__ANDROID__)
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkEnumerateInstanceLayerProperties(uint32_t* pPropertyCount, VkLayerProperties* pProperties) {
    return swapchain::vkEnumerateInstanceLayerProperties(pPropertyCount, pProperties);
}
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkEnumerateInstanceExtensionProperties(const char* pLayerName, uint32_t* pPropertyCount,
                                                                                      VkExtensionProperties* pProperties) {
    return swapchain::vkEnumerateInstanceExtensionProperties(pLayerName, pPropertyCount, pProperties);
}

VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkEnumerateDeviceLayerProperties(VkPhysicalDevice physicalDevice, uint32_t* pPropertyCount,
                                                                                VkLayerProperties* pProperties) {
    return swapchain::vkEnumerateDeviceLayerProperties(physicalDevice, pPropertyCount, pProperties);
}
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkEnumerateDeviceExtensionProperties(VkPhysicalDevice physicalDevice, const char* pLayerName,
                                                                                    uint32_t* pPropertyCount, VkExtensionProperties* pProperties) {
    return swapchain::vkEnumerateDeviceExtensionProperties(physicalDevice, pLayerName, pPropertyCount, pProperties);
}
#endif
}