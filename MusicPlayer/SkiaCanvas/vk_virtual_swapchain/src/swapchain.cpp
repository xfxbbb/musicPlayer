...
std::atomic<uint32_t> g_CpuAllocCount;
...
namespace swapchain {
	...
	namespace {
		...
		void OverrideCurrentExtentIfNecessary(VkExtent2D* current_extent) {
			...
			if (GetParameter(kOverrideSurfaceExtentEnv, kOverrideSurfaceExtentAndroidProp, &overridden_extent)) {
				std::istringstream ss(overridden_extent);
				VkExtent2D extent;
				ss >> extent.width;
				if (ss.fail()) {
					write_warning("Failed to parse surface extent parameter: " + overridden_extent);
					return;
				}
				ss >> extent.height;
				if (ss.fail()) {
	...
	void RegisterInstance(VkInstance instance, const InstanceData& data) {
		...
		data.vkEnumeratePhysicalDevices(instance, &num_devices, physical_devices.data());

		auto physical_device_map = GetGlobalContext().GetPhysicalDeviceMap();

		for (VkPhysicalDevice physical_device : physical_devices) {
			PhysicalDeviceData dat{ instance };
			data.vkGetPhysicalDeviceMemoryProperties(physical_device, &dat.memory_properties_);
			data.vkGetPhysicalDeviceProperties(physical_device, &dat.physical_device_properties_);
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkCreateVirtualSurface(VkInstance instance, const CreateNext* pCreateInfo, const VkAllocationCallbacks* pAllocator,
		...
		if (pCreateInfo != nullptr) {
			for (const CreateNext* pNext = static_cast<const CreateNext*>(pCreateInfo->pNext); pNext != nullptr;
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceSurfaceSupportKHR(VkPhysicalDevice physicalDevice, uint32_t queueFamilyIndex,
		...
		const auto instance_dat = *GetGlobalContext().GetInstanceData(GetGlobalContext().GetPhysicalDeviceData(physicalDevice)->instance_);

		for (uint32_t i = 0; i <= queueFamilyIndex; ++i) {
			uint32_t property_count = 0;
			instance_dat.vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, &property_count, nullptr);
			assert(property_count > queueFamilyIndex);

			std::vector<VkQueueFamilyProperties> properties(property_count);
			instance_dat.vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, &property_count, properties.data());
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceSurfaceCapabilitiesKHR(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface,
		...
		VkPhysicalDeviceProperties& properties = GetGlobalContext().GetPhysicalDeviceData(physicalDevice)->physical_device_properties_;
		...
		pSurfaceCapabilities->maxImageExtent = { properties.limits.maxImageDimension2D, properties.limits.maxImageDimension2D };
		pSurfaceCapabilities->maxImageArrayLayers = properties.limits.maxImageArrayLayers;
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceSurfaceCapabilities2KHR(VkPhysicalDevice physicalDevice,
		...
		return swapchain::vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice, pSurfaceInfo->surface,
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceSurfaceFormatsKHR(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface,
		...
		*pSurfaceFormatCount = 1;
		...
		pSurfaceFormats->format = VK_FORMAT_B8G8R8A8_UNORM;// VK_FORMAT_R8G8B8A8_UNORM;
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceSurfaceFormats2KHR(VkPhysicalDevice physicalDevice,
		const VkPhysicalDeviceSurfaceInfo2KHR* pSurfaceInfo,
		uint32_t* pSurfaceFormatCount, VkSurfaceFormatKHR* pSurfaceFormats) {
		return swapchain::vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, pSurfaceInfo->surface, pSurfaceFormatCount, pSurfaceFormats);
	}
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkCreateSwapchainKHR(VkDevice device, const VkSwapchainCreateInfoKHR* pCreateInfo,
		...
		PhysicalDeviceData& pdd = *GetGlobalContext().GetPhysicalDeviceData(dev_dat.physicalDevice);
		InstanceData& inst_dat = *GetGlobalContext().GetInstanceData(pdd.instance_);

		uint32_t property_count = 0;
		inst_dat.vkGetPhysicalDeviceQueueFamilyProperties(dev_dat.physicalDevice, &property_count, nullptr);

		std::vector<VkQueueFamilyProperties> queue_properties(property_count);
		inst_dat.vkGetPhysicalDeviceQueueFamilyProperties(dev_dat.physicalDevice, &property_count, queue_properties.data());
...
#ifdef MyTest
		auto swp = new VirtualSwapchain(device, queue, &pdd.physical_device_properties_, &pdd.memory_properties_, &dev_dat, pCreateInfo, &g_CpuAllocationCallbacks);
#else
		auto swp = new VirtualSwapchain(device, queue, &pdd.physical_device_properties_, &pdd.memory_properties_, &dev_dat, pCreateInfo, pAllocator);
#endif // MyTest
		...
		for (const CreateNext* pNext = static_cast<const CreateNext*>(pCreateInfo->pNext); pNext != nullptr;
			...
			if (pNext->sType == VIRTUAL_SWAPCHAIN_CREATE_PNEXT) {
				...
				if (pNext->surfaceCreateInfo) {
					swp->CreateBaseSwapchain(pdd.instance_, &inst_dat, pAllocator, pNext->surfaceCreateInfo);
				}
				break;
			}
		}
...
#ifdef MyTest
		swp->Destroy(&g_CpuAllocationCallbacks);
#else
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkGetSwapchainImagesKHR(VkDevice device, VkSwapchainKHR swapchain, uint32_t* pSwapchainImageCount,
		...
		VirtualSwapchain* swp = reinterpret_cast<VirtualSwapchain*>(swapchain);
		const auto images = swp->GetImages(*pSwapchainImageCount, pSwapchainImages != nullptr);
		if (!pSwapchainImages) {
	...
	VKAPI_ATTR void VKAPI_CALL vkSetSwapchainCallback(VkSwapchainKHR swapchain, void callback(void*, uint8_t*, size_t), void* user_data) {
		VirtualSwapchain* swp = reinterpret_cast<VirtualSwapchain*>(swapchain);
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkAcquireNextImageKHR(VkDevice device, VkSwapchainKHR swapchain, uint64_t timeout, VkSemaphore semaphore,
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkAcquireNextImage2KHR(VkDevice device, const VkAcquireNextImageInfoKHR* pAcquireInfo, uint32_t* pImageIndex) {
		...
		return swapchain::vkAcquireNextImageKHR(device, pAcquireInfo->swapchain, pAcquireInfo->timeout, pAcquireInfo->semaphore, pAcquireInfo->fence,
			pImageIndex);
	}
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkQueuePresentKHR(VkQueue queue, const VkPresentInfoKHR* pPresentInfo) {
		...
		std::vector<VkPipelineStageFlags> pipeline_stages(pPresentInfo->waitSemaphoreCount, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT);

		size_t i = 0;
		for (; i < pPresentInfo->swapchainCount; ++i) {
			uint32_t image_index = pPresentInfo->pImageIndices[i];

			VirtualSwapchain* swp = reinterpret_cast<VirtualSwapchain*>(pPresentInfo->pSwapchains[i]);
			...
			res = EXPECT_SUCCESS(GetGlobalContext().GetQueueData(queue)->vkQueueSubmit(queue, 1, &submitInfo, swp->GetFence(image_index)));
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkQueueSubmit(VkQueue queue, uint32_t submitCount, const VkSubmitInfo* pSubmits, VkFence fence) {
		...
		return GetGlobalContext().GetQueueData(queue)->vkQueueSubmit(queue, submitCount, pSubmits, fence);
	}
	...
	VKAPI_ATTR void VKAPI_CALL vkCmdPipelineBarrier(VkCommandBuffer commandBuffer, VkPipelineStageFlags srcStageMask,
		...
		PFN_vkCmdPipelineBarrier func = GetGlobalContext().GetCommandBufferData(commandBuffer)->vkCmdPipelineBarrier;

		return func(commandBuffer, srcStageMask, dstStageMask, dependencyFlags, memoryBarrierCount, pMemoryBarriers, bufferMemoryBarrierCount,
	...
	VKAPI_ATTR void VKAPI_CALL vkCmdWaitEvents(VkCommandBuffer commandBuffer, uint32_t eventCount, const VkEvent* pEvents,
		...
		PFN_vkCmdWaitEvents func = GetGlobalContext().GetCommandBufferData(commandBuffer)->vkCmdWaitEvents;

		func(commandBuffer, eventCount, pEvents, srcStageMask, dstStageMask, memoryBarrierCount, pMemoryBarriers, bufferMemoryBarrierCount,
			pBufferMemoryBarriers, imageMemoryBarrierCount, imageBarriers.data());
	}
	...
	VKAPI_ATTR VkResult VKAPI_CALL vkCreateRenderPass(VkDevice device, const VkRenderPassCreateInfo* pCreateInfo,
		...
		PFN_vkCreateRenderPass func = GetGlobalContext().GetDeviceData(device)->vkCreateRenderPass;
