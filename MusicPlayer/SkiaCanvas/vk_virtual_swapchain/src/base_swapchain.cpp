...
namespace swapchain {
    ...
    namespace {
        VkResult createSemaphore(const DeviceData* device_functions, VkDevice device, const VkAllocationCallbacks* pAllocator, VkSemaphore* pSem) {
            VkSemaphoreCreateInfo createInfo = {
                VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO, // sType
                nullptr,                                 // pNext
                0,                                       // flags
            };
            return device_functions->vkCreateSemaphore(device, &createInfo, pAllocator, pSem);
        }
        VkResult createSemaphores(const DeviceData* device_functions, VkDevice device, const VkAllocationCallbacks* pAllocator, size_t count,
                                  std::vector<VkSemaphore>& sems) {
            sems.resize(count);
            for (VkSemaphore& sem : sems) {
                VkResult res;
                if ((res = createSemaphore(device_functions, device, pAllocator, &sem)) != VK_SUCCESS) {
                    return res;
                }
            }
            return VK_SUCCESS;
        ...
        void destroySemaphores(const DeviceData* device_functions, VkDevice device, const VkAllocationCallbacks* pAllocator,
                               std::vector<VkSemaphore>& sems) {
            for (VkSemaphore sem : sems) {
    ...
    BaseSwapchain::BaseSwapchain(VkInstance instance, VkDevice device, uint32_t queue, VkCommandPool command_pool, uint32_t num_images,
                                 const InstanceData* instance_functions, const DeviceData* device_functions,
                                 const VkSwapchainCreateInfoKHR* swapchain_info, const VkAllocationCallbacks* pAllocator, const void* platform_info)
        : instance_(instance), device_(device), instance_functions_(instance_functions), device_functions_(device_functions),
          swapchain_info_(*swapchain_info), surface_(VK_NULL_HANDLE), swapchain_(VK_NULL_HANDLE), acquire_semaphore_(VK_NULL_HANDLE), valid_(false) {
        ...
        if (device_functions_->vkAllocateCommandBuffers(device, &command_buffer_info, command_buffers_.data()) != VK_SUCCESS) {
            return;
        }
        ...
        if (createSemaphore(device_functions_, device_, pAllocator, &acquire_semaphore_) != VK_SUCCESS) {
            return;
        }
        if (createSemaphores(device_functions_, device_, pAllocator, num_images, blit_semaphores_) != VK_SUCCESS) {
            return;
        }
        if (createSemaphores(device_functions_, device_, pAllocator, num_images, present_semaphores_) != VK_SUCCESS) {
            return;
        }
    ...
    VkResult BaseSwapchain::PresentFrom(VkQueue queue, size_t index, VkImage image) {
        ...
        // for VkQueueSubmit
        if ((res = device_functions_->vkAcquireNextImageKHR(device_, swapchain_, UINT64_MAX, acquire_semaphore_, VK_NULL_HANDLE, &base_index)) !=
            VK_SUCCESS) {
            return res;
        }
        ...
        device_functions_->vkCmdPipelineBarrier(cmdbuf,
                                                0,                              // srcStageMask
        ...
        device_functions_->vkCmdBlitImage(cmdbuf,
                                          image,                                // srcImage
        ...
        VkSemaphore signal_semaphores[] = {blit_semaphores_[index], present_semaphores_[index]};
        ...
        if ((res = queue_functions->vkQueueSubmit(queue, 1, &submitInfo, VK_NULL_HANDLE)) != VK_SUCCESS) {
            return res;
        }
        ...
        if ((res = queue_functions->vkQueuePresentKHR(queue, &presentInfo)) != VK_SUCCESS) {
            return res;
        }
        return VK_SUCCESS;
    }
