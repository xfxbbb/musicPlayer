...
namespace swapchain {
	...
	public:
		...
		VirtualSwapchain(VkDevice device, uint32_t queue, const VkPhysicalDeviceProperties* pProperties,
			...
			uint32_t pending_image_timeout_in_milliseconds = 10, bool always_get_acquired_image = false);
		...
		VkCommandBuffer& GetCommandBuffer(size_t i) { return image_data_[i].command_buffer_; }

		// If we have create info, create a surface to render to.
		void CreateBaseSwapchain(VkInstance instance, const InstanceData* instance_functions_, const VkAllocationCallbacks* pAllocator,
		...
		void handleSynchronizationImage(size_t pending_image);
		...
		void SetAlwaysGetAcquiredImage(bool always_get_acquired_image) { always_get_acquired_image_ = always_get_acquired_image; }
	...
	private:
		...
		struct SwapchainImageData {
			...
			VkCommandBuffer command_buffer_; // The command_buffer that contains the copy commands.
		};
		...
		std::deque<SwapchainImageData> image_data_; // All of the data for each requested swapchain image.
		std::deque<uint32_t> pending_images_;       // Indices into image_data_ for all images that
		...
		VkCommandPool command_pool_;                // The command_pool that we are allocating buffers from.
		...
		threading::mutex pending_images_lock_; // The lock for modifying our pending images list.
		...
		threading::mutex free_images_lock_; // The lock for modifying our free images list.
