...
namespace core {
    ...
    typedef void*(GetVulkanInstanceProcAddressFunc)(size_t instance, const char* name);
    typedef void*(GetVulkanDeviceProcAddressFunc)(size_t instance, size_t device, const char* name);
    ...
    template <typename VPAPROC>
    VPAPROC get_vulkan_fun(const char* name) {
        static DlLoader dylib(systemVulkanPath().c_str());
        VPAPROC vpa = reinterpret_cast<VPAPROC>(dylib.lookup(name));
        return vpa;
    }
