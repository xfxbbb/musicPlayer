...
#ifdef _WIN32
                auto time = std::chrono::duration_cast<std::chrono::milliseconds>(rel_time).count();
                return (0 != SleepConditionVariableCS(&condition_, &native_handle, time)) ? cv_status::no_timeout : cv_status::timeout;
#else
                timespec tv;
                const std::chrono::seconds sec = std::chrono::duration_cast<std::chrono::seconds>(rel_time);
                tv.tv_sec = sec.count();
                tv.tv_nsec = std::chrono::duration_cast<std::chrono::nanoseconds>(rel_time - sec).count();

                return (0 == pthread_cond_timedwait(&condition_, &native_handle, &tv)) ? cv_status::no_timeout : cv_status::timeout;
#endif
            }
